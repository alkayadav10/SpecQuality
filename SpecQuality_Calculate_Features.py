...
# Amino acid masses (from Perl script)
AMINO_ACID_MASSES = {
...
def custom_log10(n):
    """
    Custom log10 to handle non-positive inputs, aiming to mimic Perl's behavior
    where subsequent calculations often use fallbacks like 0.01.
    Perl's log(0) is -Inf, log(negative) is NaN.
    """
...
def calculate_spectral_quality_features(peaks_list, parent_mh_val, precursor_intensity_val, charge_z_val, 
    ...
    # In Perl, precursor intensity is immediately log10 transformed.
    ...
    sum_noise_intensity = 1.0  # Init to 1 to prevent div by zero, as in Perl
    ...
    for i in range(num_peaks - 1):
        ...
        for j in range(i + 1, num_peaks):
            ...
            if tolerance_unit.lower() == 'da':
                ...
                # WARNING: Perl's logic for PPM tolerance here is unusual.
    ...
    # Complements Fraction calculation from Perl
    # Perl: $complements = 0.01 if $complements <= 0;
    ...
    if not math.isfinite(complements_fraction_val) or complements_fraction_val == 0 : # Check for 0 as well, as Perl may treat small values near 0 as needing floor
...
def parse_mgf_file(mgf_filepath, peak_cutoff):
    """Parse MGF file and extract spectra data"""
    ...
    try:
        with open(mgf_filepath, 'r') as mgf_file:
            for line_num, line_content in enumerate(mgf_file, 1):
                ...
                if line.startswith('BEGIN IONS'):
                    ...
                    if len(current_spectrum_peaks) < peak_cutoff:
                        continue
                    ...
                    # Perl: $precursorMH = ($mz*$z); This is not [M+H]+, but used as 'parent_MH' in calculations
        ...
        print(f"Processed {mgf_spectra_count} spectra from MGF file")
        return all_spectra_features
...
def parse_mzml_file(mzml_filepath, peak_cutoff):
    """Parse mzML file and extract spectra data"""
    all_spectra_features = {}
    mzml_spectra_count = 0
    processed_spectra_count = 0

    print(f"Processing mzML file: {mzml_filepath}")
    try:
        run = pymzml.run.Reader(mzml_filepath)
        
        for spectrum in run:
            mzml_spectra_count += 1
            
            # Process only MS2 spectra
            if spectrum.ms_level != 2:
                continue
                
            # Get spectrum identifier - ensure it's a string
            spectrum_id = str(spectrum.ID)  # Convert to string to avoid type issues
            if not spectrum_id:
                spectrum_id = f"spectrum_{mzml_spectra_count}"
            
            # Get retention time (convert to seconds if needed)
            rt_seconds = 0.0
            if hasattr(spectrum, 'scan_time'):
                ...
                if isinstance(rt, tuple) and len(rt) == 2:
                    rt_value, rt_unit = rt
                    if rt_unit == 'minute':
                        rt_seconds = rt_value * 60
                    else:  # assume seconds
                        rt_seconds = rt_value
                else:
                    rt_seconds = float(rt) if rt else 0.0
            ...
            if hasattr(spectrum, 'selected_precursors') and spectrum.selected_precursors:
                ...
                precursor_mz = precursor.get('mz', 0.0)
                precursor_charge = precursor.get('charge', 1.0)
                precursor_intensity = precursor.get('i', 1.0)
            
            # Get peaks
            peaks = []
            if hasattr(spectrum, 'mz') and hasattr(spectrum, 'i'):
            ...
            if len(peaks) < peak_cutoff:
                continue
            
            # Calculate parent MH (mz * charge)
            calculated_parent_mh = precursor_mz * precursor_charge
            
            # Calculate features
            features_dict = calculate_spectral_quality_features(
                peaks, calculated_parent_mh, precursor_intensity,
                precursor_charge, param_tolerance, param_tolerance_unit, param_noise_threshold
            )
            
            features_dict['COL'] = [str(precursor_mz), str(precursor_charge), str(rt_seconds)]
            all_spectra_features[spectrum_id] = features_dict
            processed_spectra_count += 1
            
            # Progress reporting
            if processed_spectra_count % 1000 == 0:
                print(f"Processed {processed_spectra_count} MS2 spectra...")
                
        print(f"Processed {processed_spectra_count} MS2 spectra out of {mzml_spectra_count} total spectra from mzML file")
        return all_spectra_features
        
    except Exception as e:
        print(f"An error occurred during mzML processing: {e}", file=sys.stderr)
        sys.exit(1)

def main():
    if not (2 <= len(sys.argv) <= 3):
        print(f"Usage: python {os.path.basename(sys.argv[0])} <input_file> [msgf_plus_output_file]", file=sys.stderr)
        print("  input_file: MGF or mzML file", file=sys.stderr)
        sys.exit(1)

    input_filepath = sys.argv[1]
    msgf_filepath = sys.argv[2] if len(sys.argv) > 2 else None

    # Parameters from Perl script
    global param_tolerance, param_tolerance_unit, param_noise_threshold, param_peak_cutoff
    param_tolerance = 0.5
    ...
    # Check file type and generate output filenames
    sqs_output_filepath = ''
    psm_sqs_output_filepath = ''
    if input_filepath.lower().endswith('.mgf'):
        file_type = 'mgf'
        base_name = input_filepath[:-4] if input_filepath.lower().endswith('.mgf') else input_filepath
        sqs_output_filepath = f"{base_name}_mgf_features.tsv"
        psm_sqs_output_filepath = f"{base_name}_PSM_mgf_features.tsv"
    elif input_filepath.lower().endswith('.mzml'):
        file_type = 'mzml'
        base_name = input_filepath[:-5] if input_filepath.lower().endswith('.mzml') else input_filepath
        sqs_output_filepath = f"{base_name}_mzML_features.tsv"
        psm_sqs_output_filepath = f"{base_name}_PSM_mzML_features.tsv"
    else:
        print(f"Error: Unsupported file format. Please provide MGF (.mgf) or mzML (.mzml) file.", file=sys.stderr)
        sys.exit(1)
    
    #sqs_output_filepath = f"{base_name}_features.tsv"
    #psm_sqs_output_filepath = f"{base_name}_PSM_features.tsv"

    # Parse input file based on type
    if file_type == 'mgf':
        all_spectra_features = parse_mgf_file(input_filepath, param_peak_cutoff)
    else:  # mzml
        all_spectra_features = parse_mzml_file(input_filepath, param_peak_cutoff)

    mgf_processing_time = time.time()
    print(f"File processed in: {mgf_processing_time - script_start_time:.2f} secs ({(mgf_processing_time - script_start_time)/60:.2f} mins)")
    ...
    try:
        with open(sqs_output_filepath, 'w') as sqs_out_file:
            ...
            for scan_id, features in all_spectra_features_with_sqs.items():
                ...
                # Ensure all row elements are strings
                row = [
                    str(scan_id),  # Convert scan_id to string
                    str(col_data[0]), 
                    str(col_data[1]), 
                    str(col_data[2]),
                    f"{features.get('peak_count', 0.0):.4f}", 
                    f"{features.get('signal_peaks', 0.0):.4f}",
                    f"{features.get('snr', 0.0):.4f}", 
                    f"{features.get('intensity', 0.0):.4f}",
                    f"{features.get('peak_density', 0.0):.4f}", 
                    f"{features.get('good_diff_fraction', 0.0):.4f}",
                    f"{features.get('complements_fraction', 0.0):.4f}", 
                    f"{features.get('isotope_peaks', 0.0):.4f}",
                    f"{features.get('neutral_loss_peaks', 0.0):.4f}", 
                    f"{features.get('ari', 0.0):.4f}",
                ...
                with open(msgf_filepath, 'r') as msgf_in_file, open(psm_sqs_output_filepath, 'w') as psm_out_file:
                    for line in msgf_in_file:
                        ...
                        if scan_title_from_msgf in all_spectra_features_with_sqs:
                            ...
                            features_to_append = [
                                f"{sqs_features.get('peak_count', 0.0):.4f}", 
                                f"{sqs_features.get('signal_peaks', 0.0):.4f}",
                                f"{sqs_features.get('snr', 0.0):.4f}", 
                                f"{sqs_features.get('intensity', 0.0):.4f}",
                                f"{sqs_features.get('peak_density', 0.0):.4f}", 
                                f"{sqs_features.get('good_diff_fraction', 0.0):.4f}",
                                f"{sqs_features.get('complements_fraction', 0.0):.4f}", 
                                f"{sqs_features.get('isotope_peaks', 0.0):.4f}",
                                f"{sqs_features.get('neutral_loss_peaks', 0.0):.4f}", 
                                f"{sqs_features.get('ari', 0.0):.4f}",
                                f"{sqs_features.get('sqs', 0.0):.4f}", 
                                label
                            ]
